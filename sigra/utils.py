...
def Batch_Data(adata, num_batch_x, num_batch_y, spatial_key=["X", "Y"], plot_Stats=False):
    ...
    batch_x_coor = [
        np.percentile(Sp_df[:, 0], (1 / num_batch_x) * x * 100) for x in range(num_batch_x + 1)
    ]
    batch_y_coor = [
        np.percentile(Sp_df[:, 1], (1 / num_batch_y) * x * 100) for x in range(num_batch_y + 1)
    ]
...
def Cal_Spatial_Net(
    adata, rad_cutoff=None, k_cutoff=None, model="Radius", verbose=True, use_global=False
):
    if verbose:
