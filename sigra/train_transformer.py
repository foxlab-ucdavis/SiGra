...
def test_one_branch(
    ...
    if branch == "img_img":
        ...
        use_img_loss = 0
    model = TransImg(
        hidden_dims=[data.x.shape[1], img.x.shape[1]] + hidden_dims, use_img_loss=use_img_loss
    ).to(device)
    # KP: guessing this is correct but not sure
    load_path = os.path.join(save_path, "final_%d.pth" % (repeat))
    print(load_path)
...
def test_img(
    ...
    with torch.no_grad():
        ...
        sc.pl.umap(
            adata_Vars, color=["Ground Truth"], show=False, title="combined latent variables"
        )
...
def test_nano_batch(
    ...
    loader = NeighborLoader(data, num_neighbors=[5, 5], batch_size=512, shuffle=False)
    model.eval()
    czs = None
    for batch in loader:
...
def train_nano_batch(
    ...
    data.x = torch.cat([data.x, img.x], dim=1)
    # data = data.cuda()
    loader = NeighborLoader(data, num_neighbors=[5, 5], batch_size=512, shuffle=True)
    ...
    for epoch in range(1, n_epochs + 1):
        for bid, batch in enumerate(loader):
            ...
            print(
                "epoch: %d, batch: %d, loss: %.2f, gloss: %.2f, iloss: %.2f, closs: %.2f"
                % (epoch, bid, loss, gloss, iloss, closs)
            )
            loss.backward()
...
def test_nano_fov_ablation(
    ...
    if use_img_loss:
        ...
        model = TransImg(
            hidden_dims=[gene_dim, img_dim] + hidden_dims, use_img_loss=use_img_loss
        ).to(device)
    if model_name is not None:
        model.load_state_dict(torch.load(os.path.join(save_path, model_name)))
    else:
...
def train_nano_fov_ablation(
    ...
    for epoch in tqdm(range(1, n_epochs + 1)):
        ...
        if epoch > 500 and epoch % 100 == 0:
            # if epoch > 1:
            torch.save(
                model.state_dict(), os.path.join(save_path, "final_%d_%d.pth" % (epoch, repeat))
            )
...
def train_nano(
    ...
    for epoch in tqdm(range(1, n_epochs + 1)):
        ...
        if epoch > 500 and epoch % 100 == 0:
            torch.save(
                model.state_dict(), os.path.join(save_path, "final_%d_%d.pth" % (repeat, epoch))
            )
...
def train_one_branch(
    ...
    with torch.no_grad():
        model.load_state_dict(
            torch.load(os.path.join(save_path, "final_%d_%s.pth" % (repeat, branch)))
        )
...
def train_10x_all(
    ...
    model = TransImg(hidden_dims=[gene_dim, img_dim] + hidden_dims, use_img_loss=use_img_loss).to(
        device
    )
