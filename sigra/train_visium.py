...
def infer(opt, result_path="../results/10x_final/"):
    ...
    sc.pp.log1p(adata)
    Ann_df = pd.read_csv(
        "%s/%s/annotation.txt" % (opt.root, opt.id), sep="\t", header=None, index_col=0
    )
    ...
    sc.pl.spatial(adata, color=["Ground Truth"], title=["Ground Truth"], show=False)
    ...
    plt.rcParams["figure.figsize"] = (3, 3)
    sc.settings.figdir = save_path
    sc.pl.spatial(adata, color=["mclust"], title=["scGIT (ARI=%.2f)" % (ARI)], show=False)
    ...
    plt.rcParams["figure.figsize"] = (3, 3)
    sc.settings.figdir = save_path
    sc.pl.umap(adata, color=["Ground Truth"], show=False, title="scGIT", legend_loc="on data")
    ...
    sc.pl.stacked_violin(
        adata, plot_genes, groupby="mclust", ax=axs[0], title="sigra_RAW", show=False, vmax=0.3
    )  # , save='Violin_raw.pdf')
    sc.pl.stacked_violin(
        adata,
        plot_genes,
