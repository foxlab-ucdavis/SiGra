...
def draw_all_fovs(image, transformation_matrix, fov, z_index="zIndex_3", btype="DAPI"):
    ...
    for inst_cell in meta_cell.index.tolist():
        ...
        cv2.imwrite(
            os.path.join("../dataset/merscope/cut_images/%s" % (btype), inst_cell + "_%s.tif")
            % (z_index),
            sub_img,
        )
...
def split_fovs():
    ...
    for xstart in range(gxmin, gxmax, xinterval):
        ...
        for ystart in range(gymin, gymax, yinterval):
            idx = (
                (all_data.obs["globalx"] >= xstart)
                & (all_data.obs["globalx"] <= (xstart + xinterval))
                & (all_data.obs["globaly"] >= ystart)
                & (all_data.obs["globaly"] <= (ystart + xinterval))
            )
            ...
            plt.savefig(
                "../dataset/merscope/fov_images/fov_%d_%d.png" % (row, col), bbox_inches="tight"
            )
...
def processing_mscope():
    ...
    save_image(
        root="../dataset/merscope/Liver1Slice1_images_mosaic_DAPI_z3.tif",
        transform_root="../dataset/merscope/Liver1Slice1_images_micron_to_mosaic_pixel_transform.csv",
        btype="DAPI",
    )
    save_image(
        root="../dataset/merscope/Liver1Slice1_images_mosaic_Cellbound1_z3.tif",
        transform_root="../dataset/merscope/Liver1Slice1_images_micron_to_mosaic_pixel_transform.csv",
        btype="bound1",
    )
    save_image(
        root="../dataset/merscope/Liver1Slice1_images_mosaic_Cellbound2_z3.tif",
        transform_root="../dataset/merscope/Liver1Slice1_images_micron_to_mosaic_pixel_transform.csv",
        btype="bound2",
    )
    save_image(
        root="../dataset/merscope/Liver1Slice1_images_mosaic_Cellbound3_z3.tif",
        transform_root="../dataset/merscope/Liver1Slice1_images_micron_to_mosaic_pixel_transform.csv",
        btype="bound3",
    )
